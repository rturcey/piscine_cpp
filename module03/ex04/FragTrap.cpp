...
/*   Updated: 2021/01/06 08:07:43 by rturcey          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
FragTrap::FragTrap(void)
{
	...
	ranged = 20;
	armor = 5;
	std::cout << "CONSTRUCTOR CALLED :: [DEFAULT_FRAG] This time it'll be awesome, I promise!" << std::endl;
	return;
}
...
unsigned int	FragTrap::vaulthunter_dot_exe(std::string const &target)
{
	...
	std::cout << this->type << " " << this->getName() << " attacks " << target << " with " << attacks[no];
	std::cout << ", causing " << dmg[no] << " points of damage !" << std::endl;
	this->energy -= 25;
	std::cout << this->type << " " << this->getName() << "'s energy is now " << this->energy << "/" << this->energy_max << std::endl;
	return (dmg[no]);
}
